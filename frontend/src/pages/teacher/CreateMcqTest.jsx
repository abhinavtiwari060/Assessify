...
const CreateMcqTest = ({ isEditMode = false }) => {
  ...
  const handleAddComprehensionSection = () => {
    const newPassageId = `pass_${Date.now()}_${Math.random().toString(36).substr(2, 5)}`;
    setQuestions([
      ...questions,
      {
        type: 'comprehension',
  ...
  // Add Sub-Question to existing Passage
  const handleAddQuestionToPassage = (targetPassageId) => {
    const newQuestions = [...questions];
    let lastIdx = -1;
    for (let i = newQuestions.length - 1; i >= 0; i--) {
      if (newQuestions[i].passageId === targetPassageId) {
        lastIdx = i;
        break;
      }
    }
    ...
    if (lastIdx !== -1) {
      newQuestions.splice(lastIdx + 1, 0, newSubQ);
    } else {
      newQuestions.push(newSubQ);
    }
    setQuestions(newQuestions);
  };

  // Remove single question by global index
  const handleRemoveQuestion = (idx) => {
    ...
    // If removing the passage holder, assign passage text to next item in same passageId
    if (targetQ.passageId && targetQ.passage) {
      const nextMatching = updated.find((q) => q.passageId === targetQ.passageId);
      if (nextMatching) {
        nextMatching.passage = targetQ.passage;
      }
    }
  ...
  // Remove entire comprehension group
  const handleRemovePassageSection = (targetPassageId) => {
    if (questions.length <= questions.filter((q) => q.passageId === targetPassageId).length) {
      addToast('Test must contain at least 1 question', 'warning');
      return;
    }
    setQuestions(questions.filter((q) => q.passageId !== targetPassageId));
  };

  // Update passage text for a passage group
  const handlePassageTextChange = (targetPassageId, passageText) => {
    const updated = [...questions];
    let passageAssigned = false;

    updated.forEach((q) => {
      if (q.passageId === targetPassageId) {
        if (!passageAssigned) {
          q.passage = passageText;
          passageAssigned = true;
        } else {
          q.passage = '';
        }
      }
  ...
  const handleSubmit = async (e) => {
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      if (q.type === 'comprehension' && q.passageId) {
        const firstP = questions.find((p) => p.passageId === q.passageId && p.passage.trim());
        if (!firstP || !firstP.passage.trim()) {
          addToast(`Reading Comprehension section has an empty passage`, 'error');
          return;
        }
      }
  ...
  // Group questions by passageId for visual rendering
  const renderQuestionBlocks = () => {
    ...
    questions.forEach((q, qIdx) => {
      if (q.type === 'comprehension' && q.passageId) {
        if (processedPassages.has(q.passageId)) return;
        processedPassages.add(q.passageId);

        // Find all sub-questions for this passage
        const groupSubQs = questions
          .map((subQ, originalIdx) => ({ subQ, originalIdx }))
          .filter(({ subQ }) => subQ.passageId === q.passageId);

        const firstInGroup = groupSubQs.find(({ subQ }) => subQ.passage) || groupSubQs[0];
        const passageText = firstInGroup ? firstInGroup.subQ.passage : '';

        blocks.push(
          <div
            key={`passage_block_${q.passageId}`}
            className="bg-[var(--bg-card)] border-2 border-[#FA8128]/40 rounded-2xl p-6 sm:p-8 space-y-6 shadow-sm relative"
          >
            {/* Comprehension Section Banner */}
            <div className="flex items-center justify-between border-b border-[var(--border)] pb-4">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-[var(--text-sub)]">
                    {groupSubQs.length} Question{groupSubQs.length > 1 ? 's' : ''} under this passage
                  </p>
                </div>
              </div>
              ...
              <button
                type="button"
                onClick={() => handleRemovePassageSection(q.passageId)}
                className="flex items-center gap-1 px-3 py-1.5 rounded-xl bg-rose-500/10 text-rose-600 dark:text-rose-400 hover:bg-rose-500/20 text-xs font-bold transition-colors cursor-pointer"
              >
                <Trash2 className="w-3.5 h-3.5" /> Remove Passage Section
              </button>
            </div>

            {/* Passage Textarea */}
            <div className="space-y-1.5">
              <label className="text-[11px] font-bold uppercase tracking-wider text-[#FA8128] flex items-center gap-1.5">
                <FileText className="w-3.5 h-3.5" /> Passage / Reference Text
              </label>
              <textarea
                rows={5}
                required
                value={passageText}
                onChange={(e) => handlePassageTextChange(q.passageId, e.target.value)}
                placeholder="Enter or paste the reading comprehension passage here..."
            ...
            <div className="space-y-6 pt-2">
              ...
              {groupSubQs.map(({ subQ, originalIdx }, subIndex) => (
                <div
                  key={`subQ_${originalIdx}`}
                  className="bg-[var(--bg-sub)] rounded-xl p-5 border border-[var(--border)] space-y-4"
                >
                  <div className="flex items-center justify-between border-b border-[var(--border)] pb-2.5">
                    <span className="font-extrabold text-xs text-[var(--text-main)] flex items-center gap-2">
                      <HelpCircle className="w-4 h-4 text-[#FA8128]" />
                      Question #{originalIdx + 1} (Passage Q{subIndex + 1})
                    </span>
                    ...
                    <div className="flex items-center gap-3">
                      <div className="flex items-center gap-1.5 text-xs font-semibold text-[var(--text-sub)]">
                        ...
                        <input
                          ...
                          min={1}
                          value={subQ.marks}
                          onChange={(e) => handleMarksChange(originalIdx, e.target.value)}
                          className="w-14 bg-[var(--bg-card)] border border-[var(--border)] rounded-lg px-2 py-0.5 text-center font-bold text-[var(--text-main)]"
                        />
                      </div>
                      ...
                      <button
                        ...
                        className="p-1 text-[#EF4444] hover:bg-[var(--bg-card)] rounded-lg transition-colors cursor-pointer"
                  ...
                  <div className="space-y-1">
                    ...
                    <textarea
                      ...
                      onChange={(e) => handleQuestionTextChange(originalIdx, e.target.value)}
                      placeholder="Enter question statement related to the passage..."
                      className="w-full bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-3 text-xs font-medium text-[var(--text-main)] focus:outline-none focus:ring-2 focus:ring-[#FA8128]"
                    />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                      {subQ.options.map((opt, optIdx) => {
                        const isCorrect = subQ.correctAnswerIndex === optIdx;
                        return (
                          <div
                            key={optIdx}
                          ...
                          >
                            <input
                              type="radio"
                              name={`correct-ans-${originalIdx}`}
                              checked={isCorrect}
                              onChange={() => handleCorrectAnsChange(originalIdx, optIdx)}
                              className="w-4 h-4 text-[#22C55E] focus:ring-[#22C55E] cursor-pointer"
                            />
                            <span className="w-5 h-5 rounded-md bg-[var(--bg-sub)] border border-[var(--border)] text-[10px] font-extrabold flex items-center justify-center text-[var(--text-main)]">
                            ...
                            <input
                              ...
                              className="flex-1 bg-transparent border-none text-xs font-medium text-[var(--text-main)] focus:outline-none"
                            />
                          </div>
                        );
                      })}
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      onChange={(e) => handleExplanationChange(originalIdx, e.target.value)}
                      placeholder="Explanation based on the passage..."
                      className="w-full bg-[var(--bg-card)] border border-[var(--border)] rounded-xl px-3 py-1.5 text-xs text-[var(--text-main)]"
                    />
                  </div>
                </div>
              ))}
              ...
              <button
                type="button"
                onClick={() => handleAddQuestionToPassage(q.passageId)}
                className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-[#FA8128]/15 hover:bg-[#FA8128]/25 text-[#FA8128] font-bold text-xs cursor-pointer border border-[#FA8128]/30 transition-all"
              >
                <PlusCircle className="w-4 h-4 text-[#FA8128]" /> Add Question to Passage
              </button>
            </div>
          </div>
        );
      } else if (q.type === 'mcq' || !q.passageId) {
        blocks.push(
          <div
            key={`q_card_${qIdx}`}
    ...
    <form onSubmit={handleSubmit} className="space-y-6 max-w-5xl mx-auto pb-12 text-[var(--text-main)]">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-[var(--text-main)] tracking-tight">
            {isEditMode ? 'Edit MCQ / Comprehension Test' : 'Create New MCQ / Comprehension Test'}
          </h1>
          <p className="text-xs sm:text-sm text-[var(--text-sub)] mt-1">
            Configure test parameters, reading comprehension passages, timers, and question options.
          </p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-1">
            ...
            <input
              ...
              placeholder="e.g. Core Java & Reading Comprehension Test"
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={handleAddNormalQuestion}
              className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-[var(--bg-sub)] hover:bg-[var(--bg-card-hover)] border border-[var(--border)] text-[#FA8128] font-bold text-xs cursor-pointer transition-all"
            >
              <PlusCircle className="w-4 h-4 text-[#FA8128]" /> Add Normal MCQ
            </button>

            <button
              type="button"
              onClick={handleAddComprehensionSection}
              className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-[#FA8128] hover:bg-[#E06D1A] text-white font-bold text-xs shadow-xs cursor-pointer transition-all"
            >
              <BookOpen className="w-4 h-4 text-white" /> Add Reading Comprehension
            </button>
          </div>
        </div>

        {/* Render Question Cards / Comprehension Blocks */}
      ...
      <div className="flex justify-end gap-4 pt-4">
        ...
        >
          ...
          <span>{saving ? 'Saving Test...' : isEditMode ? 'Update Test' : 'Publish Test'}</span>
        </button>
      </div>
    </form>
  );
